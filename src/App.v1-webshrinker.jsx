...
const PortfolioCard = ({ item, index }) => {
  ...
  useEffect(() => {
    const cacheKey = `portfolio_screenshot_${index}`;
    
    // Tenta recuperar do cache
    const cachedImage = localStorage.getItem(cacheKey);
    if (cachedImage && cachedImage !== 'fallback') {
      setImageUrl(cachedImage);
      setImageLoaded(true);
      return;
    }

    // Se falhou antes, mostra fallback direto
    ...
    // Para os 3 últimos sites usa Webshrinker (sem limite, sem autenticação)
    const isNewSite = item.url.includes('tankasapkota') || 
                      item.url.includes('sekko') || 
                      item.url.includes('pregariaecompanhia');
    
    const url = isNewSite 
      ? `https://images.webthumbnail.org/?url=${encodeURIComponent(item.url)}&width=600&height=800`
      : `https://image.thum.io/get/width/600/crop/800/noanimate/${item.url}`;
  ...
  const handleImageLoad = () => {
    ...
    const cacheKey = `portfolio_screenshot_${index}`;
    if (imageUrl && imageUrl.length < 5000) { // Evita salvar URLs muito longas
  ...
  const handleImageError = () => {
    setShowFallback(true);
    setImageLoaded(true);
    // Marca como falho no cache para próximas tentativas
    const cacheKey = `portfolio_screenshot_${index}`;
  ...
  return (
    ...
    >
      <div className="aspect-video bg-slate-900 relative overflow-hidden">
        ...
        <div className={`absolute inset-0 bg-gradient-to-br from-blue-600 to-purple-600 ${showFallback || !imageLoaded ? 'flex' : 'hidden'} items-center justify-center`}>
          <Globe size={48} className="text-white opacity-50 group-hover:opacity-100 transition" />
        </div>
